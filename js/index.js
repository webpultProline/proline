		...
		$('#'+idBlockParent).find('.proline_tab').each(function(){
			if($(this).attr('id') == idBlock){
				...
				if($(this).find('.slider').length > 0){
					var self_slide = $(this).find('.slider');
					self_slide.removeClass('loaded');
					self_slide.slick('slickGoTo',0);
					self_slide.on('afterChange', function(){
						self_slide.addClass('loaded');
					});
				}
			} else {
	...
	/*3d анимация*/
	function renderObject(e, t, a) {
		window.addEventListener("resize", function() {
			camera.aspect = e.clientWidth / e.clientHeight, camera.updateProjectionMatrix(), r.setSize(e.clientWidth, e.clientHeight), o()
		}, !1);
		var i = e.clientWidth,
			n = e.clientHeight;
		e.setAttribute("width", i), e.setAttribute("height", n);
		var r = new THREE.WebGLRenderer({
			canvas: e
		});
		window.renderer = r;
		r.setClearColor(3355443), r.setPixelRatio(window.devicePixelRatio), r.setSize(i, n), r.shadowMap.enabled = !0, r.shadowMap.type = THREE.PCFSoftShadowMap, scene = new THREE.Scene, scene.background = new THREE.Color('rgb(255,255,255)'), camera = new THREE.PerspectiveCamera(35, e.clientWidth / e.clientHeight, 1, 5e3), camera.up.set(0, .5, 1), camera.position.set(0, 0, 1500), scene.add(camera);
		
		var d = new THREE.OrbitControls(camera, r.domElement);
		
		d.addEventListener("change", o), d.minDistance = 500, d.maxDistance = 2000, d.enablePan = !0, d.target.set(0, 0, 0), d.update(), dirLight = new THREE.DirectionalLight(16774337, 1), dirLight.color.setHSL(.1, 1, .95), dirLight.position.set(50, 50, 150), dirLight.position.multiplyScalar(1), scene.add(dirLight), scene.add(new THREE.AmbientLight(16777215, .5)), dirLight.castShadow = !0, dirLight.shadow.mapSize.width = 2048, dirLight.shadow.mapSize.height = 2048;
		
		dirLight.shadow.camera.left = -50, dirLight.shadow.camera.right = 50, dirLight.shadow.camera.top = 50, dirLight.shadow.camera.bottom = -50, dirLight.shadow.camera.far = 4500, dirLight.shadow.bias = -1e-4;
		
		new THREE.LoadingManager;
		function o() {
			r.render(scene, camera)
		}(new THREE.MTLLoader).setPath(t).load(a + ".mtl", function(e) {
            e.preload(), (new THREE.OBJLoader).setMaterials(e).setPath(t).load(a + ".obj", function(e) {
				window.object = e;
				window.scene = scene;
				window.camera = camera;
                e.position.y = -200, e.castShadow = !1, e.receiveShadow = !1, scene.add(e)
            })
        }),
        function e() {
            r.render(scene, camera), requestAnimationFrame(function() {
                e()
            })
        }()
	}
	...
	function animationStart(){
		...
		window.object.rotation.y += 0.025;
		
		window.renderer.render(window.scene, window.camera);
		window.animation_3d_roate = requestAnimationFrame(function(){
			animationStart();
		});
	}
	function animationStop(){
		var cancelAnimationFrame = window.cancelAnimationFrame || window.mozCancelAnimationFrame;
		cancelAnimationFrame(window.animation_3d_roate);
	}
	
	function plusScene(){
		var fov = window.camera.fov;
		if(fov >= 10){
			window.camera.fov = fov - 10;
...
$(function(){
	/*навигационные кнопки 3d*/
	$('.btn-view3d').click(function(){
		if($(this).hasClass('active_rotate') == false){
			$(this).addClass('active_rotate');
			animationStart();
		} else {
			$(this).removeClass('active_rotate');
			animationStop();
		}
	});
	$('.btn-view3d--plus').click(function(){
		plusScene();
	});
	$('.btn-view3d--minus').click(function(){
		minusScene();
	});
	/*конец*/
	/*тест авторизации*/
	...
	*/
			...
			if($(window).width() < 1200){
				...
				if(scrollTop > position_carousel){
					if($('.slider.slider-proline.slider-proline_smallCard').hasClass('scrolled') == false){
						$('.comparison-carousel_on_top-main').css({
							'height':'auto',
						});
				...
				//
				//
				var position_carousel = $('.comparison-carousel_on_top-main').offset().top-($('.header-fixed').height()+22);
				
				var self_parent = $('.proline_product-characteristics_list .proline_product-characteristics_before-row');
				
				var padding_left = self_parent.find('span').first().offset().left-(parseInt($('.comparison-bottom').css('padding-left'),10) + 5);
				
				if(scrollTop > position_carousel){
					if($('.comparison-carousel_on_top').hasClass('scrolled') == false){
						$('.slider.slider-proline.slider-proline_smallCard').removeClass('scrolled');
						$('.slider.slider-proline.slider-proline_smallCard').css({
							'width':'100%',
							'left':'0px'
						...
						$('.comparison-carousel_on_top-main').css({
							'height':$('.comparison-carousel_on_top').height()+'px',
						});
						$('.comparison-carousel_on_top').css({
							'top':$('.header-fixed').height()+'px',
							'padding-left':padding_left+'px',
							'padding-right':padding_left+'px',
							'width':'100%'
						});
						$('.comparison-carousel_on_top').addClass('scrolled');
					}
				} else {
					if($('.comparison-carousel_on_top').hasClass('scrolled') == true){
						$('.slider.slider-proline.slider-proline_smallCard').removeClass('scrolled');
						$('.slider.slider-proline.slider-proline_smallCard').css({
							'width':'100%',
							'left':'0px'
						...
						$('.comparison-carousel_on_top').removeClass('scrolled');
					}
				}
				if($('.slider-proline_smallCard').hasClass('slick-initialized') == true){
					$('.slider-proline_smallCard').slick('slickGoTo',0);
	...
	function changeSizeForDesktop_comparison(){
	...
	$(window).resize(function(){
		if($('.comparison-rightside').length > 0){
			fixedComparisonSlider();
			changeSizeForDesktop_comparison();
			resizeBlockSMallSlider();
		}
	});
	//
		...
		$('.mobile-select-region').on('touchmove', function(event){
			if($('.mobile-select-region-btn').hasClass('open') == true){
				if($(window).width() < 992){
					...
					if(nowDeltaX > 5 && window.stopActionForSelectRegion == false){
	...
	*/
	...
	попапы
	*/
	$('.drop__form').on('click', function(event){
		if($(this).hasClass('drop') == false){
			custom_selectEvents(true,true,$(this),'dropped__form');
			$(this).addClass('drop');
		} else {
			if($(event.target).parents().hasClass('dropped__form') == false){
				custom_selectEvents(false,true,$(this),'dropped__form');
		...
		if($(this).find('.quality_counter').length > 0){
			var checked_input = $(this).find('input:checked').length;
			if(checked_input > 0){
				if($(this).find('.quality_counter').css('display') == 'none'){
					$(this).find('.quality_counter').stop().fadeIn();
				}
				$(this).find('.quality_counter').text('');
				$(this).find('.quality_counter').text('+'+checked_input);
			} else {
				$(this).find('.quality_counter').stop().fadeOut();
			}
		}
	});
		...
		if(window.custom_select == true){
			if($(event.target).parents().hasClass(window.custom_select_parent_class) == false && $(event.target).hasClass(window.custom_select_parent_class) == false){
				if(window.custom_select_block == false){
					...
					if(window.custom_select_parent_class == 'dropped__form'){
						window.custom_select_parent.removeClass('drop');
					}
				}
			}
